...
export default function SectionNavigator() {
  ...
  return (
    <>
      <div className="sticky top-0 z-40 border-b border-border bg-surface/95 px-4 py-3 backdrop-blur lg:hidden">
        <label
          className="mx-auto flex max-w-7xl items-center gap-3 text-sm text-body"
          htmlFor="section-navigator"
        >
          <span className="shrink-0 font-medium text-muted">Viewing:</span>
          <span className="relative min-w-0 flex-1">
            <select
              id="section-navigator"
              className="min-h-11 w-full appearance-none rounded-lg border border-control-border bg-surface py-2 pl-3 pr-10 text-sm font-medium text-heading outline-none focus:border-focus focus:ring-2 focus:ring-focus"
              value={activeSection}
            ...
            <svg
              className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-heading"
              viewBox="0 0 16 16"
              fill="none"
              aria-hidden="true"
            >
      ...
      {showDesktopRail && (
        <nav
          className="group fixed left-0 top-1/2 z-40 hidden w-60 -translate-x-40 -translate-y-1/2 rounded-r-xl border border-l-0 border-border bg-surface/95 p-2 shadow-xl shadow-heading/10 backdrop-blur transition-transform duration-200 hover:translate-x-0 focus-within:translate-x-0 motion-reduce:transition-none lg:block"
          aria-label="Page sections"
        >
          <a
            href="#top"
            onClick={(event) => handleLinkClick(event, "top")}
            className={`grid grid-cols-[1fr_5rem] items-center rounded-md text-xs transition focus:outline-none focus:ring-2 focus:ring-inset focus:ring-focus ${
              activeSection === "top"
                ? "bg-ai-surface font-semibold text-heading"
                : "text-muted hover:bg-section hover:text-heading"
            }`}
            ...
            {roleSections.map((section) => {
              ...
              return (
                <a
                  ...
                  className={`grid grid-cols-[1fr_5rem] items-center rounded-md text-xs leading-4 transition focus:outline-none focus:ring-2 focus:ring-inset focus:ring-focus ${
                    isActive
                      ? "bg-ai-surface font-semibold text-heading"
                      : "text-muted hover:bg-section hover:text-heading"
                  }`}
          ...
          <div className="mt-1 border-t border-ai-border pt-1">
